...
class SotbitCrosssellGlobalCondCtrl
{
...
class SotbitCrosssellGlobalCondCtrlComplex extends SotbitCrosssellGlobalCondCtrl
{
    public static function GetControlDescr()
    {
        $strClassName = get_called_class();
...
class SotbitCrosssellGlobalCondCtrlAtoms extends SotbitCrosssellGlobalCondCtrl
{
    /**
     * @return array|bool
     */
...
class SotbitCrosssellGlobalCondCtrlGroup extends SotbitCrosssellGlobalCondCtrl
{
    public static function GetControlDescr()
    {
        $className = get_called_class();
...
class SotbitCrosssellCatalogCondCtrl extends SotbitCrosssellGlobalCondCtrl
{
}

class SotbitCrosssellCatalogCondCtrlComplex extends SotbitCrosssellGlobalCondCtrlComplex
{
}

class SotbitCrosssellCatalogCondCtrlGroup extends SotbitCrosssellGlobalCondCtrlGroup
{
    public static function GetControlDescr()
    {
...
class SotbitCrosssellCatalogCondCtrlIBlockFields extends SotbitCrosssellCatalogCondCtrlComplex
{
    public static function GetControlDescr()
    {
        $description = parent::GetControlDescr();
    ...
    public static function GetControls($strControlID = false)
    {
        ...
        foreach ($arControlList as &$control)
        {
            ...
            $control['MODULE_ID'] = 'sotbit.crosssell';
...
class SotbitCrosssellCatalogCondCtrlIBlockProps extends SotbitCrosssellCatalogCondCtrlComplex
{
    public static function GetControlDescr()
    {
        $description = parent::GetControlDescr();
    ...
    public static function GetControls($strControlID = false, $iblockId = null)
    {
        ...
        {
            $arControlList['CondIBPrice'.$arPriceType['ID']] = array(
                'PARENT' => false,
                'EXIST_HANDLER' => 'Y',
                'MODULE_ID' => 'catalog',
                'MODULE_ENTITY' => 'iblock',
                'ENTITY' => 'ELEMENT_PROPERTY',
                'IBLOCK_ID' => $arPriceType['ID'],
                'MULTIPLE' => 'Y',
                'GROUP' => 'N',
                'SEP' => $boolSep ? 'Y' : 'N',
                'SEP_LABEL' => Loc::getMessage('BT_MOD_COND_PRICES_BLOCK'),

                'ID' => 'CondIBPrice'.$arPriceType['ID'],
                'FIELD' => 'PRICE'.$arPriceType['ID'],
                'FIELD_TYPE' => 'int',
                'LABEL' => $arPriceType['NAME_LANG'] . " [" . $arPriceType['ID'] . "]",
                'PREFIX' => $arPriceType['NAME_LANG'] . " [" . $arPriceType['ID'] . "]",
        ...
        if (!empty($arIBlockList))
        {
            ...
            foreach ($arIBlockList as &$intIBlockID)
            {
                ...
                {
                    $boolSep = true;
                    //$rsProps = CIBlockProperty::GetList(array('SORT' => 'ASC', 'NAME' => 'ASC'), array('IBLOCK_ID' => $intIBlockID, 'PROPERTY_TYPE' => 'E'));
...
class SotbitCrosssellGlobalCondTree
{
    ...
    protected function GetEventList($intEventID)
    {
        $arEventList = array(
            BT_COND_BUILD_CATALOG => array(
                ...
                'CONTROLS' => array(
                    'MODULE_ID' => 'sotbit.crosssell',
                    'EVENT_ID' => 'OnCondCatControlBuildListSotbitCrosssell'
                )
            ),
...
class SotbitCrosssellCatalogCondTree extends SotbitCrosssellGlobalCondTree
{
    ...
    public function checkingForEmptiness($arrCondition){
        foreach ($arrCondition as $key => $item) {
            if(is_array($item))
                self::checkingForEmptiness($item);
            else if($key == 'value' && empty($item))
                self::$empty = false;
        }
        return self::$empty;
    }
