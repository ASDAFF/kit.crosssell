...
use \Sotbit\Crosssell\Helper\crosssellaction;
...
if($APPLICATION->GetGroupRight('sotbit.crosssell') == "D")
...
{
    if(isset($_REQUEST['action_button_sotbit_crosssell_options']))
    {
        if($_REQUEST['action_button_sotbit_crosssell_options'] == 'delete')
        {
            if(is_array($_POST['ID'])) {
                CrosssellAction::groupAction($_POST['ID'], $_REQUEST['action_button_sotbit_crosssell_options']);
            } else {
                // burger Delete
                CrosssellAction::simpleAction($_REQUEST['ID'], $_REQUEST['action_button_sotbit_crosssell_options'], $_REQUEST['type']);
            }
        }
        if($_REQUEST['action_button_sotbit_crosssell_options'] == 'edit') {
            CrosssellAction::groupAction($_POST['FIELDS'], $_REQUEST['action_button_sotbit_crosssell_options']);
        }
    }
    ...
    if(isset($_REQUEST['delete']))
    {
        ...
        LocalRedirect('/bitrix/admin/sotbit_crosssell_list.php?lang='.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : ''));
    }
...
if( \SotbitCrosssell::ReturnDemo() == 2){
    ?>
    <div class="adm-info-message-wrap adm-info-message-red">
        <div class="adm-info-message">
            <div class="adm-info-message-title"><?=Loc::getMessage("CROSSSELL_DEMO")?></div>
            <div class="adm-info-message-icon"></div>
        </div>
    </div>
    <?
}
if( \SotbitCrosssell::ReturnDemo() == 3 || \SotbitCrosssell::ReturnDemo() == 0)
{
    ?>
    <div class="adm-info-message-wrap adm-info-message-red">
        <div class="adm-info-message">
            <div class="adm-info-message-title"><?=Loc::getMessage("CROSSSELL_DEMO_END")?></div>
            <div class="adm-info-message-icon"></div>
        </div>
    </div>
    <?
...
$sTableID = "sotbit_crosssell_options";
...
if (isset($_REQUEST['CATEGORY_ID']) && ($_REQUEST['CATEGORY_ID'] != ''))
{
    $aMenu= array(
        array(
            ...
            "LINK" => '/bitrix/admin/sotbit_crosssell_list.php?lang='.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : ''),
            "ICON" => "btn_list"
        )
    );
...
$rsData->NavStart();
$lAdmin->SetNavigationParams($rsData, array("BASE_LINK" => '/bitrix/admin/sotbit_crosssell_list.php'));
...
while($arRes = $rsData->NavNext(false))
{
    ...
    if($itemType == "S") {
        ...
        $url_section = '/bitrix/admin/sotbit_crosssell_list.php?'.implode('&', $arLink);
    } else {
        ...
        $url_element = '/bitrix/admin/sotbit_crosssell.php?'.implode('&', $arLink);
    }
    ...
    if($itemType == "S")
        $row->AddViewField($itemType."ID", '<a href="/bitrix/admin/sotbit_crosssell_category.php?lang=ru&site='.$_REQUEST['site'].'&ID='.$arRes['ID'].(isset($_REQUEST['CATEGORY_ID']) ? '&CATEGORY_ID='.$_REQUEST['category'] : '').'" title="'.GetMessage("IBLIST_A_EDIT").'">'.$itemId.'</a>');
    else
        $row->AddViewField("ID", '<a href="/bitrix/admin/sotbit_crosssell.php?lang=ru&site='.$_REQUEST['site'].'&ID='.$arRes['ID'].(isset($_REQUEST['CATEGORY_ID']) ? '&CATEGORY_ID='.$_REQUEST['category'] : '').
    ...
    if($APPLICATION->GetGroupRight('sotbit.crosssell') == "W") {
        ...
        $arActions[] = array(
            ...
            "ACTION" => $lAdmin->ActionRedirect('/bitrix/admin/sotbit_crosssell'.(($itemType == 'S') ? '_category' : '').'.php?lang=ru&site='.$_REQUEST['site'] .
...
$aContext[] = array(
    ...
    "LINK" => '/bitrix/admin/sotbit_crosssell.php?' .implode('&', $arrLink) . '&TYPE=CROSSSELL'
);
$aContext[] = array(
...
$aContext[] = array(
    ...
    "ICON" => ($boolBtnNew ? "" : "btn_new"),
    "LINK" => '/bitrix/admin/sotbit_crosssell.php?' .implode('&', $arrLink) . '&TYPE=COLLECTION'
);
$aContext[] = array(
    "TEXT" => Loc::getMessage($langPrefix.'CREATE_CATEGORY'),
    "ICON" => ($boolBtnNew ? "" : "btn_new"),
    "LINK" => '/bitrix/admin/sotbit_crosssell_category.php?' .implode('&', $arrLink) . '&TYPE=CROSSSELL'
);
...
if(CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_edit") && !defined("CATALOG_PRODUCT")) {
    //echo BeginNote(), 'test node', ' <a href="'.htmlspecialcharsbx('/bitrix/admin/sotbit.opengraph_url_list.php?&lang='.LANGUAGE_ID.'&admin=Y&return_url='.urlencode("iblock_list_admin.php?".$sThisSectionUrl)).'">', 'test link', '</a>', EndNote();
}
