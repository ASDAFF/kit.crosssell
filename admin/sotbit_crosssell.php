...
use Kit\Crosssell\Orm\CrosssellProductsTable;
use Kit\Crosssell\Helper;
...
if(isset($_REQUEST['ID'])) {
    ...
    $arResult = $rs->fetch();
    $obCond4 = new \KitCrosssellCatalogCondTree();
...
if($_POST['save'] != "" || $_POST['apply'] != "") {
    $obCond3 = new \KitCrosssellCatalogCondTree();
    ...
    if($_POST['crosssell_tab1_sort'] != null & $_POST['NAME'] != null & count($_POST['crosssell_tab1_site']) > 0 ) {
        ...
        if(is_array($_POST['crosssell_tab1_detailed_img']))
        {
            ...
            $secondImg = CFile::SaveFile($arELEMENT_FILE, "kit.crosssell");
        ...
        \KitCrosssellCatalogCondTree::setEmptyFlag();
        // Checking if condition is empty // ----------------------------------------------------------------------------------------------------------------------
        if($TYPE == 'COLLECTION') {
            if(\KitCrosssellCatalogCondTree::checkingForEmptiness($cond1['CHILDREN']) == false) {
...
//            }
        ...
        if($isSafe) {
            ...
            if(isset($_REQUEST['ID'])) {
                ...
                if($result) {
                    if($_REQUEST['apply']) {
                        LocalRedirect("/bitrix/admin/kit_crosssell.php?lang=" . $lang . "&site=" . $siteId ."&ID=" . $_REQUEST['ID'] . '&TYPE=' . $TYPE . '&SUCCESS=Y&CATEGORY_ID='.$CATEGORY_ID);
                    }
                    if($_REQUEST['save']) {
                        LocalRedirect("/bitrix/admin/kit_crosssell_list.php?lang=" . $lang . "&site=" . $siteId .'&CATEGORY_ID='. $CATEGORY_ID);
                    }
                } else {
                ...
                if($result->isSuccess()) {
                    if($_REQUEST['apply']) {
                        LocalRedirect("/bitrix/admin/kit_crosssell.php?lang=" . $lang . "&site=" . $siteId ."&ID=" . $ID . '&TYPE=' . $TYPE . (isset($_REQUEST["CATEGORY_ID"]) ? '&CATEGORY_ID='.$_REQUEST["CATEGORY_ID"] : ''));
                        echo Helper\CrosssellNotice::getSuccess(Loc::getMessage("CROSSELL_PAGE_SUCCESS_SAVED"));
                    }
                    if($_REQUEST['save']) {
                        LocalRedirect("/bitrix/admin/kit_crosssell_list.php?lang=" . $lang . "&site=" . $siteId . (isset($_REQUEST["CATEGORY_ID"]) ? '&CATEGORY_ID='.$_REQUEST["CATEGORY_ID"] : ''));
                    }
                } else {
                    echo Helper\CrosssellNotice::getError(Loc::getMessage("CROSSELL_PAGE_ERROR_SAVE_TO_DB"));
                }
...
if($APPLICATION->GetGroupRight('kit.crosssell') == "W")
{
    if($TYPE == 'CROSSSELL') {
        $aMenu= array(
            array(
                ...
                "LINK" => '/bitrix/admin/kit_crosssell_list.php?lang='.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : '') . (isset($_REQUEST['CATEGORY_ID']) ? '&CATEGORY_ID='.$_REQUEST['CATEGORY_ID'].'&apply_filter=Y' : ''),
                "ICON" => "btn_list"
            ),
        ...
        {
            $aMenu[] = array(
                "TEXT" => GetMessage( "KIT_CROSSSELL_GENERATE" ),
                "TITLE" => GetMessage( "KIT_CROSSSELL_GENERATE" ),
                "ICON" => "btn_generate",
    ...
    if($_REQUEST['ID'])
    {
        $urlDelete = '/bitrix/admin/kit_crosssell_list.php?lang'.SITE_ID.(isset($_REQUEST['site']) ? '&site='.$_REQUEST['site'] : '').'&sessid='.bitrix_sessid().'&type=E&delete='.$_REQUEST['ID'];

        $aMenu[] = array(
            "TEXT" => GetMessage( "KIT_CROSSSELL_DELETE_BLOCK"),
            "ONCLICK" => "javascript:if(confirm('".GetMessageJS("KIT_CROSSSELL_NEW_CATEGORY_DELETE_BLOCK_TEXT")."'))top.window.location.href='".CUtil::JSEscape($urlDelete)."'",
            "ICON" => "btn_delete",
        );
    }
            ...
            $(document).ready(function() {
                ...
                $('.adm-detail-toolbar-right:first').prepend(`
                    <div class="generate_group" style="position: relative;">
                        <input id="generate" type="submit" value="<?= Loc::GetMessage( "KIT_CROSSSELL_GENERATE" )?>">
                    </div>
                `);
                <?endif;?>
                        ...
                        success: function (response) {
                            $(".adm-detail-toolbar-right:first").prepend(`
                                <div class="success-msg">
                                  ...
                                  <?= Loc::GetMessage( "KIT_CROSSSELL_GENERATE_SUC" )?> ${response}
                                </div>
                             `);
